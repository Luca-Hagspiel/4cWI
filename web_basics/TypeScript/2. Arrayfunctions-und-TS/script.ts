...
// 2) filter
const adults: Person[] = people.filter((p: Person): boolean => p.age >= 18);
console.log("filter - adults:", adults.map(p => p.name));

// 3) sort
const peopleByAge: Person[] = [...people].sort((a: Person, b: Person): number => a.age - b.age);
console.log("sort - people by age:", peopleByAge.map(p => `${p.name}(${p.age})`));

// 4) find
const firstTeen: Person | undefined = people.find((p: Person): boolean => p.age < 20);
console.log("find - first teen:", firstTeen);

// 5) reduce
const sum: number = nums.reduce((acc: number, cur: number): number => acc + cur, 0);
console.log("reduce - sum:", sum);

// 6) some
const hasMinor: boolean = people.some((p: Person): boolean => p.age < 18);
